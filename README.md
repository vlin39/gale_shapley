...
The **stable matching problem** involves finding a way to pair individuals from two different set, A and B, which are usually of equal size, based on ranked preferences, such that no two individuals would rather be with each other than with their assigned partners. Each individual ranks every member of the opposite group in order of preference.
